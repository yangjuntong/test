<template>
  <div class="pay">
    <my-header title="确认订单">
   	...
   	<div class="buy">
   		<div>购买商品</div>
   		<div class="div2" v-for="(item,index) in getShopCar">
   			<img v-bind:src="getShopCar[index].getDetail.src">
   			<div class="div1">
   				<p>{{getShopCar[index].getDetail.txt}}</p>
   				<span >￥<span class="money">{{getShopCar[index].getDetail.mon}}</span></span>
   				<strong>x
   				<strong class="many">{{getShopCar[index].getDetail.many}}</strong></strong>
   			</div>
   		</div>
   		<div class="div3">
   			备注：<input type="" name="" placeholder="请输入">
   		</div>
   		<div class="div4">
   			<strong>合计：￥</strong>
   			<span id="all"></span>
